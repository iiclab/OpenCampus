...
class TfPoseEstimator:
    ...
    def inference(self, npimg, resize_to_default=True, upsample_size=1.0):
        ...
        #visualize_inter_layer(heatMat_up[0])
        vis_blender(img, heatMat_up)
...
def vis_blender(ori_img, ori_layer, heat_ratio=0.5):
    img = ori_img.copy()
    layer = ori_layer.copy()
    layer = (layer * 255).astype(np.uint8)
    ih, iw, _ = img.shape
    _, lh, lw, lc = layer.shape
    
    size_h, size_w = lh, lw
    img = cv2.resize(img, (size_w, size_h))
    line_num = math.ceil(math.sqrt(lc))
    out_canvas = np.zeros((size_h * line_num, size_w * line_num, 3), dtype=np.uint8)
    
    """
    part_name = ["鼻", "首", "右肩", "右肘", "右手首",
                 "左肩", "左肘", "左手首", "右腰", "右ひざ",
                 "右足首", "左腰", "左ひざ", "左足首", "右目",
                 "左目", "右耳", "左耳", "背景"]
    """
    part_name = ["Nose", "Neck", "RShoulder", "RElbow", "RWrist",
    ...
    for c in range(lc):
        heat = layer[0, ..., c]
        heat = cv2.resize(heat, (size_w, size_h))
        heat = cv2.applyColorMap(heat, cv2.COLORMAP_HOT)
        out = cv2.addWeighted(img, 1-heat_ratio, 
	                          heat, heat_ratio, 0)
        y = c // line_num
        x = c % line_num
        out_canvas[size_h*y: size_h*(y+1), size_w*x:size_w*(x+1)] = out
        cv2.putText(out_canvas, part_name[c],
                   (size_w*x, size_h*y+12), cv2.FONT_HERSHEY_PLAIN, 1,(255,255,0))

    cv2.imshow("heatMap", out_canvas)
    #cv2.waitKey(0)
    #cv2.show()
